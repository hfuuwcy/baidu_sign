...
推荐在仓库的 `Settings` -> `Secrets and variables` -> `Actions` -> `Secrets` 中按字段新增：

- `BAIDUWP_COOKIE`：完整 Cookie 字符串，例如 `BDUSS=xxxxxx; STOKEN=xxxxxx; ...`
- `BAIDUWP_APP_Z`：App 抓包里的 `z`
- `BAIDUWP_APP_CUID`：App 抓包里的 `cuid`
- `BAIDUWP_APP_DEVUID`：App 抓包里的 `devuid`
- `BAIDUWP_APP_UK`：`/api/taskscore/tasksave` 的 `uk`
- `BAIDUWP_APP_TOKEN`：`/api/taskscore/tasksave` 的 `token`

非敏感或低敏感字段可以放在 `Settings` -> `Secrets and variables` -> `Actions` -> `Variables`：

- `CHECKIN_MODE=app`：定时任务运行 App 端入口
- `BAIDUWP_APP_ENABLED=true`
- `BAIDUWP_APP_CHANNEL=["android_15_xxx_bd-netdisk_xxx"]`
- `BAIDUWP_APP_VERSION=13.20.7`
- `BAIDUWP_APP_VERSIONCODE=4062`
- `BAIDUWP_APP_TASKS=[{"enabled":true,"name":"App广告观看任务","task_id":"3434632741761030","task_from":["task_sys_task_growth"]}]`

邮箱也可以拆分配置，常用字段为 `BAIDUWP_EMAIL_ENABLED`、`BAIDUWP_EMAIL_SMTP_HOST`、`BAIDUWP_EMAIL_SMTP_PORT`、`BAIDUWP_EMAIL_SMTP_USER`、`BAIDUWP_EMAIL_SMTP_PASSWORD`、`BAIDUWP_EMAIL_FROM_ADDR`、`BAIDUWP_EMAIL_TO_ADDRS`。

拆分配置完整支持这些前缀字段：

```text
BAIDUWP_COOKIE
BAIDUWP_APP_ENABLED / Z / CUID / DEVUID / CHANNEL / VERSION / VERSIONCODE / CLIENTTYPE / THEMEINFO / RCHANNEL / APP / USER_AGENT / OFFLINEPACKAGE
BAIDUWP_APP_TASKS / TASKSCORE_TASKS
BAIDUWP_APP_COIN_SIGN_TASK_ID / COIN_SIGN_TASK_FROM / IS_GROWTH
BAIDUWP_APP_QUESTION_TASK_ID / QUESTION_TASK_FROM
BAIDUWP_APP_UK / TOKEN
BAIDUWP_APP_AD_WATCH_ENABLED / AD_WATCH_TASK_ID / AD_WATCH_TASK_FROM / AD_WATCH_DELAY_SECONDS（旧广告任务兼容字段）
BAIDUWP_EMAIL_ENABLED / SMTP_HOST / SMTP_PORT / SMTP_USER / SMTP_PASSWORD / SMTP_PASSWORD_ENV / FROM_ADDR / TO_ADDRS / USE_SSL / USE_TLS / SUBJECT
```
...
配置优先级为：`BAIDUWP_CONFIG` / `BAIDUWP_CONFIG_JSON` -> 拆分环境变量 `BAIDUWP_COOKIE` + `BAIDUWP_APP_*` -> `BAIDUWP`。
拆分环境变量适合单账号；多账号仍建议使用 `BAIDUWP_CONFIG`。
...
如需让定时任务运行 App 端入口，可在 Variables 中新增：
...
GET https://pan.baidu.com/api/taskscore/tasksave
```
...
`/api/taskscore/tasksave` 用于上报任务完成；多个任务可以放在 `taskscore_tasks` 列表中显式启用。
...
如果抓包里 `channel`、`task_from` 这类参数显示为数组，可以在 JSON 配置里直接写数组；脚本会按紧凑 JSON 字符串发送，例如 `["task_sys_task_growth"]`。

`taskscore_tasks` 中每个任务只需要配置自己的 `name`、`task_id`、`task_from` 和可选 `delay_seconds`。`uk`、`token`、`z`、`cuid`、`devuid` 是同一账号的 App 级参数，会被这些任务共享。某个任务失败时只会在结果里显示该任务失败，不会影响原有 App 签到和答题。
...
- 环境变量 `BAIDUWP_COOKIE` 和 `BAIDUWP_APP_*` 组成的拆分配置
- 当前目录下的 `config.json`
